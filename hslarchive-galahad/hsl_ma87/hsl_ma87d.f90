...
subroutine MA87_analyse_double(n, ptr, row, order, keep, control, info)
  ...
  !** integer :: t_start, t_end, t_rate
   ...
   select case (info78)
   case (-1) 
      go to 490 ! allocation error
   case (-2)
      info%flag = MA87_ERROR_NOT_POSDEF
...
subroutine MA87_factor_double(n, ptr, row, val, order, keep, control, info)
 ...
 !**  end if
   ...
   keep%info%num_zero      = info%num_zero
...
subroutine MA87_solve_mult_double(nrhs,lx,x,order,keep, &
   ...
   info%num_zero        = keep%info%num_zero
   info%stat            = keep%info%stat
...
subroutine factorize_posdef(n, val, order, keep, control, info, nrhs, ldr, rhs)
   ...
   integer, dimension(:), allocatable :: num_zero ! per thread zero count
...
!$ total_threads = omp_get_max_threads()
 ...
 !**  end if
   ...
   if(st.ne.0) go to 10
   deallocate (keep%zero_flag,stat=st)
   allocate (keep%zero_flag(keep%n),stat=st)
   if(st.ne.0) go to 10
 ...
 !**  call system_clock(t_start, t_rate)
   ...
   deallocate(num_zero,stat=st)
   allocate(num_zero(0:total_threads-1),stat=st)
...
!$OMP    num_zero, order, rhs, rhs_local, total_threads, stack, val &
...
!$OMP END SINGLE NOWAIT
      ...
      call task_dispatch(keep%nbcol, keep%lfact, keep%zero_flag, map, stack, &
         keep%blocks, keep%nodes, control, keep%flag_array(this_thread), st, &
         nrhs, ldr, rhs, total_threads, rhs_local, keep%maxmn, &
         detlog(this_thread), num_zero(this_thread))
...
!$OMP SINGLE
         ...
         info%num_zero = sum(num_zero(:))
!$OMP END SINGLE
!$OMP END PARALLEL
      if(info%num_zero.gt.0 .and. info%flag.ge.0) &
         info%flag = MA87_WARNING_SEMI_DEFINITE
         ...
         do while(blk.le.keep%nodes(node)%blk_en)
            ...
            do blk = blk, keep%blocks(blk)%last_blk
               ...
               if(blk .eq. keep%blocks(blk)%dblk) then
                  ...
                  keep%blocks(blk)%dep = int(keep%blocks(blk)%last_blk - blk)
               endif
               do idx = idx, idx+keep%blocks(blk)%blkm-1
                  j = map(keep%nodes(node)%index(idx))
                  if(last.ne.j) then
   ...
   do node = nodes(fnode)%least_desc, fnode
      ...
      do blk = bsa, ben
         ...
         do j = nodes(node)%sa, nodes(node)%en, nodes(node)%nb
            dblk = dblk + kk
            if(kk.lt.0) exit ! quit if we try to pass the diagonal
...
! Main worker routine
!
subroutine task_dispatch(nbcol, lfact, zero_flag, map, stack, blocks, nodes, &
   ...
   type(lfactor), dimension(nbcol), intent(inout) :: lfact ! Entries in 
      ! block columns of L
   logical, dimension(:), intent(inout) :: zero_flag ! zero_flag(i) is true
      ! if col i should be zeroed out
...
!$ this_thread = omp_get_thread_num()
  ...
  !       "Thread ", this_thread, " joined worker pool."
   ...
   num_zero = 0          ! number of pivots in (diag_zero_minus, diag_zero_plus]
...
!$OMP FLUSH(lfact)
      ...
      case(TASK_FACTORIZE_BLOCK) ! Factorize diagonal block
         ...
         call factor_diag_block(n, m, id, lfact(bcol)%lcol(sa:sa+n*m-1),   &
            zero_flag(dsa:den), control, flag, detlog, num_zero)
...
!$OMP FLUSH
     ...
     !%%%    endif
      ...
      case(TASK_SOLVE_BLOCK) ! Solve column block with diagonal (_trsm)
         ...
         sa1 = blocks(dblk)%sa
         id1 = blocks(dblk)%id
         dnode => nodes(blocks(dblk)%node)
         dcol = blocks(dblk)%bcol - blocks(dnode%blk_sa)%bcol + 1
         dsa  = dnode%sa + (dcol-1)*dnode%nb
         den  = min(dsa+dnode%nb-1, dnode%en)
         ...
         call solv_col_block(m, n, id, lfact(bcol)%lcol(sa:sa+n*m-1), &
              id1, lfact(bcol)%lcol(sa1:sa1+n*m1), zero_flag(dsa:den), &
              control)
         ...
         if(nrhs.gt.0) then
           ...
           offset = 1 + (dcol-1)*dnode%nb + int(blk-dblk)*dnode%nb
...
integer function calc_col(node, block)
   ...
   calc_col = calc_col - int(block%last_blk - block%dblk + 1) + 1 ! col of node
...
bcols: do
        ...
        ! Be careful as cptr need not be at the start of a block
         ...
         do blk = rblk, blocks(rblk)%last_blk
            ...
            rb = int(blk - blocks(blk)%dblk) + scol ! block index 
...
subroutine solve_task_dispatch(nbcol, lfact, map, npi, stack, blocks, nodes, &
   ...
   integer(long) :: blk ! task%dest
   integer :: col ! global index of fist col. in dcol
   integer(long) :: dblk ! diagonal block of current column
   integer :: dcol ! local block column index
...
!$OMP FLUSH
      ...
      case(TASK_SLV_FUPD) ! Forward update with off-diagonal
         ...
         offset   = offset + int(blk-blocks(blk)%dblk)*nodes(node)%nb ! this blk
...
! dest <- dest diag^-1
!
subroutine solv_col_block(m, n, id, dest, id1, diag, zero_flag, control)
   ...
   logical, dimension(n), intent(in) :: zero_flag
   type(MA87_control), intent(in) :: control
 ...
 !%%%  integer :: t_start, t_end, this_thread
  ...
  !%%% if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(control%diag_zero_minus.lt.control%diag_zero_plus) then
      ! May need to zero out a column
      do i = 1, n
         if(zero_flag(i)) dest(i : m*n : n) = 0.0
      end do
   endif
...
!
! TASK_FACTORIZE_BLOCK (uses Lapack routine _potrf and _trsm)
! A_ii <- L_ii
!
subroutine factor_diag_block(n, m, id, dest, zero_flag, control, dpotrf_info, &
      detlog, num_zero)
   ...
   logical, dimension(*), intent(out) :: zero_flag ! zero_flag(i) to be set to
      ! true if col i is to be zeroed out
   type(MA87_control), intent(in) :: control
 ...
 !%%%  if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(control%diag_zero_minus.ge.control%diag_zero_plus .and. &
         control%diag_zero_plus.eq.0.0_wp) then
      ! Call LAPACK routine
      call dpotrf('Upper', n, dest, n, dpotrf_info)
   else
      ! Call custom routine (slower)
      call tol_potrf(n, dest, n, zero_flag, control%diag_zero_minus, &
         control%diag_zero_plus, dpotrf_info, num_zero)
   endif
   ! check for errors
   ...
   if(m.gt.n) then
      ...
      if(control%diag_zero_minus.lt.control%diag_zero_plus) then
         ! May need to zero out a column
         do i = 1, n
            if(zero_flag(i)) dest(n*n+i : m*n : n) = 0.0
         end do
      endif
   endif
...
! Perform Cholesky factor with tolerance.
! There are three cases for diagonal entries d_ii:
! diag_zero_plus <  d_ii                        +ive eigenvalue - fine
! diag_zero_minus < d_ii <= diag_zero_plus     zero eigenvalue - set to e_i
! d_ii < min(diag_zero_minus, diag_zero_plus)   -ive eigenvalue - error
recursive subroutine tol_potrf(n, a, lda, zero_flag, diag_zero_minus, &
      diag_zero_plus, info, num_zero)
   integer, intent(in) :: n ! size of square matrix a
   real(wp), dimension(*), intent(inout) :: a ! a in row-major order
   integer, intent(in) :: lda ! leading dimension of a
   logical, dimension(n), intent(out) :: zero_flag ! set to true if a_ii in
      ! range (diag_zero_minus, diag_zero_plus] and hence col to be zeroed out
   real(wp), intent(in) :: diag_zero_minus
   real(wp), intent(in) :: diag_zero_plus
   integer, intent(out) :: info ! has same meaning as dpotrf info
   integer, intent(inout) :: num_zero ! running count of number of zeroes

   integer :: i, j
   real(wp) :: d_ii

   integer, parameter :: nb = 8 ! we recurse until n <= nb

   if(n.le.nb) then
      ! Non-recursive code
      info = 0 ! default to no problem

      ! Loop over columns
      do i = 1, n
         ! Pivot check
         d_ii = a((i-1)*lda+i)
         if(d_ii.le.diag_zero_plus) then
            ! Zero or -ive eigenvalue
            if(d_ii.le.diag_zero_minus) then
               ! Negative eigenvalue - report error
               info = i
               return
            else
               ! Zero eigenvalue - set column to e_i
               a((i-1)*lda+i) = 1.0
               a(i*lda + i : n*lda : lda) = 0.0_wp
               num_zero = num_zero + 1
               zero_flag(i) = .true.
               cycle ! NB no update required
            endif
         endif
         ! Pivot OK - apply pivot
         a((i-1)*lda+i) = sqrt(d_ii)
         d_ii = 1.0 / a((i-1)*lda+i)
         zero_flag(i) = .false.
         a(i*lda+i : n*lda : lda) = d_ii * a(i*lda+i : n*lda : lda)
         ! Update trailing submatrix
         call dsyrk('U', 'T', (n-i), 1, -1.0_wp, a(i*lda+i), lda, 1.0_wp, &
            a(i*lda+i+1), lda)
      end do
   else
      ! Recurse
      i = n / 2 ! Width of first block
      
      ! Factor A_11 (recurse)
      call tol_potrf(i, a, lda, zero_flag(1:i), diag_zero_minus, &
         diag_zero_plus, info, num_zero)
      if(info.ne.0) return
      
      ! Apply to A_21
      call dtrsm('L', 'U', 'T', 'N', i, n-i, 1.0_wp, a, lda, a(i*lda+1), lda)
      do j = 1, i
         if(zero_flag(j)) a(i*lda+j : n*lda : lda) = 0.0
      end do

      ! Update A_22
      call dsyrk('U', 'T', n-i, i, -1.0_wp, a(i*lda+1), lda, 1.0_wp, &
         a(i*lda+i+1), lda)

      ! Factor A_22 (recurse)
      call tol_potrf(n-i, a(i*lda+i+1), lda, zero_flag(i+1:n), diag_zero_minus,&
         diag_zero_plus, info, num_zero)
      if(info.ne.0) then
         info = info + i
         return
...
subroutine update_between(m, n, blk, dnode, n1, src, snode, dest, csrc,    &
 ...
 10   if (st.ne.0) then
   ...
   i = dcol + int(blocks(blk)%id - blocks(blk)%dblk) ! block in snode
   ...
   i = int(blk - blocks(blk)%dblk) + 1 ! row block of blk column
