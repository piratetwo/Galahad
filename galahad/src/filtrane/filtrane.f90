...
!===============================================================================
      ...
      SUBROUTINE FILTRANE_read_specfile( device, control, inform, alt_specname )
...
!           If desired, an alternative name for the section in the specfile
...
!     Arguments
      ...
      TYPE ( NLPT_problem_type ), TARGET, INTENT( INOUT ) :: problem
...
!------------------------------------------------------------------------------
      ...
      IF ( problem%m > 0 ) THEN
         ...
         IF ( .NOT. ALLOCATED(  problem%c ) ) THEN
            inform%status = UNASSOCIATED_INPUT
         ...
         IF ( .NOT. control%external_J_products ) THEN
            ...
            IF ( .NOT. ALLOCATED(  problem%J_val ) ) THEN
               inform%status = UNASSOCIATED_INPUT
      ...
      IF ( .NOT. ALLOCATED(  problem%g ) ) THEN
         inform%status = UNASSOCIATED_INPUT
...
!             ......................
              ...
              ELSE IF ( s%GLTR_info%status == GALAHAD_warning_on_boundary .OR. &
                        s%GLTR_info%status == GALAHAD_error_max_iterations .OR. &
                        s%GLTR_info%status == GALAHAD_ok ) THEN
