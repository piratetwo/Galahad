...
!NOT95USE GALAHAD_CPU_time
      ...
      USE GALAHAD_QPP_double, CQP_dims_type => QPP_dims_type
      USE GALAHAD_QPD_double, CQP_data_type => QPD_data_type,                  &
                              CQP_AX => QPD_AX, CQP_HX => QPD_HX,              &
...
!-*-*-*-*-*-   C Q P _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-*
      ...
      SUBROUTINE CQP_initialize( data, control, inform )
...
!  output-line-prefix                                ""
! END CQP SPECIFICATIONS (DEFAULT)
...
!  Local variables
      ...
      CHARACTER( LEN = 3 ), PARAMETER :: specname = 'CQP'
...
!-*-*-*-*-*-*-*-*-*-   C Q P _ S O L V E  S U B R O U T I N E   -*-*-*-*-*-*-*

      SUBROUTINE CQP_solve( prob, data, control, inform, C_stat, B_stat )
...
!  special case: no free variables
        ...
        IF ( prob%n == 0 ) THEN
          ...
          prob%Z( : prob%n ) = zero
          prob%C( : prob%m ) = zero
          CALL CQP_AX( prob%m, prob%C( : prob%m ), prob%m,                     &
...
!  End of CQP_solve
      ...
      END SUBROUTINE CQP_solve

!-*-*-*-*-*-   C Q P _ S O L V E _ M A I N   S U B R O U T I N E   -*-*-*-*-*

      SUBROUTINE CQP_solve_main( dims, n, m, A_val, A_col, A_ptr,              &
...
!  Arguments:
!
!  dims is a structure of type CQP_data_type, whose components hold SCALAR
...
!     LOGICAL :: generate_sif = .TRUE.
      ...
      IF ( control%out > 0 .AND. control%print_level >= 5 )                    &
        WRITE( control%out, "( A, ' entering CQP_solve_main ' )" ) prefix
...
!  test to see if we are feasible
      ...
      IF ( inform%feasible ) THEN
        ...
        IF ( Hessian_kind == 0 .AND. gradient_kind == 0 )                      &
          inform%potential = CQP_potential_value( dims, n,                     &
                                   X, DIST_X_l, DIST_X_u, DIST_C_l, DIST_C_u )
      END IF

!  compute the gradient of the Lagrangian function.

      CALL CQP_Lagrangian_gradient( dims, n, m, X, Y, Y_l, Y_u, Z_l, Z_u,      &
...
!  evaluate the merit function
      ...
      merit = CQP_merit_value( dims, n, m, X, Y, Y_l, Y_u, Z_l, Z_u,           &
                               DIST_X_l, DIST_X_u, DIST_C_l, DIST_C_u,         &
...
!   | primal optimality | <= MAX( stop_rel * | typical value |, stop_abs )
        ...
        IF ( m > 0 ) THEN 
          ...
          RHS( dims%c_e + dims%c_l_start : dims%c_e +  dims%c_u_end ) =        &
            ABS( SCALE_C * C )
          CALL CQP_abs_AX( m, RHS( dims%y_s : dims%y_e ), m, a_ne,             &
...
!  evaluate abs(dual)
        ...
        ELSE IF ( gradient_kind /= 0 ) THEN
          RHS( : n ) = RHS( : n ) + ABS( G )
        END IF
        CALL CQP_abs_AX( n, RHS( : n ), m, a_ne, A_val, A_col, A_ptr, m, Y,    &
                          'T' )
        dual_nonopt = 0
        DO i = 1, dims%x_free
          IF ( ABS( GRAD_L( i ) ) >                                            &
...
!  lies in a given wide neighbourhood of the central path
          ...
          CALL CQP_compute_lmaxstep( dims, n, m, nbnds, X, X_l, X_u, DX_zh,    &
...
!  compute the best point on the arc and its complementarity
          ...
          CALL CQP_compute_v_alpha( dims, n, m, iorder, X_coef, C_coef, Y_coef,&
...
!  evaluate the lunge
          ...
          CALL CQP_compute_v_alpha( dims, n, m, order, X_coef, C_coef,         &
                           Y_coef, Y_l_coef, Y_u_coef, Z_l_coef, Z_u_coef,     &
                           X, X_l, X_u, Z_l, Z_u, Y, Y_l, Y_u, C,              &
                           C_l, C_u, one, comp )
          ...
          IF ( .NOT. inform%feasible ) THEN
            ...
            IF ( Hessian_kind == 0 .AND. gradient_kind == 0 ) THEN
              IF ( slkmin_x >= epsmch .AND. slkmin_c >= epsmch ) THEN
                inform%potential = CQP_potential_value( dims, n, X, DIST_X_l,  &
...
!  End of CQP_solve_main

      END SUBROUTINE CQP_solve_main

!-*-*-*-*-*-*-   C Q P _ T E R M I N A T E   S U B R O U T I N E   -*-*-*-*-*

      SUBROUTINE CQP_terminate( data, control, inform )
...
!  Arguments:
!
!   data    see Subroutine CQP_initialize
!   control see Subroutine CQP_initialize
!   inform  see Subroutine CQP_solve
...
!  Deallocate all arrays allocated within SBLS
      ...
      IF ( inform%SBLS_inform%status /= 0 ) THEN
        ...
        inform%bad_alloc = 'cqp: data%SBLS'
...
!  Deallocate all arrays allocated for the preprocessing stage
      ...
      IF ( data%QPP_inform%status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate
        inform%alloc_status = data%QPP_inform%alloc_status
        inform%bad_alloc = 'cqp: data%QPP'
        IF ( control%deallocate_error_fatal ) RETURN
      END IF

!  Deallocate all remaing allocated arrays

      array_name = 'cqp: data%INDEX_C_freed'
      ...
      array_name = 'cqp: data%A_sbls%row'
      ...
      array_name = 'cqp: data%A_sbls%col'
      ...
      array_name = 'cqp: data%A_sbls%val'
      ...
      array_name = 'cqp: data%H_sbls%col'
...
!  End of function CQP_merit_value
      ...
      END FUNCTION CQP_merit_value

!-*-*-*-  C Q P _ P O T E N T I A L _ V A L U E   S U B R O U T I N E  -*-*-*-

      FUNCTION CQP_potential_value( dims, n, X, DIST_X_l, DIST_X_u,            &
                                    DIST_C_l, DIST_C_u )
      REAL ( KIND = wp ) CQP_potential_value
...
!write(6, "( ' grad_l ', /, ( 5ES12.4) )" ) GRAD_l
      ...
      CALL CQP_AX( n, GRAD_L, m, a_ne, A_val, A_col, A_ptr, m, Y, '-T' )
...
!  End of CQP_Lagrangian_gradient
      ...
      END SUBROUTINE CQP_Lagrangian_gradient

!-*-*-*-  C Q P _ C O M P U T E _ S T E P S I Z E   S U B R O U T I N E  -*-*-*-

      SUBROUTINE CQP_compute_stepsize( dims, n, m, nbnds, order, puiseux,      &
                                       X_coef, C_coef, Y_coef, Y_l_coef,       &
                                       Y_u_coef, Z_l_coef, Z_u_coef,           &
                                       X, X_l, X_u, Z_l, Z_u,                  &
                                       Y, Y_l, Y_u, C, C_l, C_u,               &
...
!  finally, compute the best point on the arc and its complementarity
      ...
      CALL CQP_compute_v_alpha( dims, n, m, order, X_coef, C_coef, Y_coef,     &
                                Y_l_coef, Y_u_coef, Z_l_coef, Z_u_coef,        &
                                X, X_l, X_u, Z_l, Z_u, Y, Y_l, Y_u, C,         &
                                C_l, C_u, alpha_max, comp )
      RETURN

!  End of subroutine CQP_compute_stepsize

      END SUBROUTINE CQP_compute_stepsize

!-*-*-*-  C Q P _ C O M P U T E _ V _ A L P H A   S U B R O U T I N E  -*-*-*-

      SUBROUTINE CQP_compute_v_alpha( dims, n, m, order, X_coef, C_coef,       &
...
!  End of subroutine CQP_compute_v_alpha

      END SUBROUTINE CQP_compute_v_alpha
...
!-*-*-*-  C Q P _ C O M P U T E _ P M A X S T E P   S U B R O U T I N E  -*-*-*-

      SUBROUTINE CQP_compute_pmaxstep( dims, n, m, nbnds, order, puiseux,      &
...
!  End of CQP_indicators
      ...
      END SUBROUTINE CQP_indicators

!-*-*-*-*-*-*-   C Q P _ w o r k s p a c e   S U B R O U T I N E  -*-*-*-*-*-*-

      SUBROUTINE CQP_workspace( m, n, dims, a_ne, h_ne, Hessian_kind,          &
...
!  End of subroutine CQP_workspace
