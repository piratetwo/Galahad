   PROGRAM GALAHAD_GLRT_EXAMPLE  !  GALAHAD 2.4 - 15/05/2010 AT 14:30 GMT.
   ...
   DO                                    !  Iteration to find the minimizer
     ...
     CASE ( 0 )  !  Successful return
       ...
       WRITE( 6, "( ' objective recurred and calculated = ', 2ES16.8 )" )       &
        inform%obj, 0.5_working * DOT_PRODUCT( X, H_vector ) + SUM( X ) +       &
        ( sigma / p ) * ( two * DOT_PRODUCT( X + O, X + O ) + eps ) ** ( p/two )
