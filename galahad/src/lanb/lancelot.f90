! THIS VERSION: GALAHAD 2.6 - 17/06/2014 AT 07:20 GMT.
...
!  exit if the elapsed-time limit has been exceeded
           ...
           IF ( control%cpu_time_limit > zero .AND.                           &
                tim - S%time > control%cpu_time_limit ) THEN
             IF ( S%printi ) WRITE( S%out, "( /, ' Time limit exceeded ' )" ) 
             inform%status = 19 ; RETURN
           END IF
